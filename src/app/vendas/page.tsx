...
export default function NegociacoesPage() {
  const { negociacoes, loading, updateNegociacaoStatus, createNegociacao } =
    useNegociacoes();
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [isNewNegotiationOpen, setIsNewNegotiationOpen] = useState(false);
  const [filterData, setFilterData] = useState({} as any);
  const [newNegotiationForm, setNewNegotiationForm] =
    useState<NewNegotiationForm>(initialFormData);
