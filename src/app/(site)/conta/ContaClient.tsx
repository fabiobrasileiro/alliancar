...
interface ContaClientProps {
  ...
  initialVendas: Venda[];
  initialAfiliados: AfiliadoInfo[];
  initialSelectedAfiliadoId: string;
  isAdmin: boolean;
}
...
export default function ContaClient({
  ...
  initialVendas,
  initialAfiliados,
  initialSelectedAfiliadoId,
  isAdmin,
}: ContaClientProps) {
  const [dashboard, setDashboard] = useState<DashboardData | null>(initialDashboard);
  const [allVendas, setAllVendas] = useState<Venda[]>(initialVendas);
  const [vendas, setVendas] = useState<Venda[]>(initialVendas);
  const [afiliados, setAfiliados] = useState<AfiliadoInfo[]>(initialAfiliados);
  ...
  const [afiliadoSelecionado, setAfiliadoSelecionado] = useState<string>(initialSelectedAfiliadoId);
  ...
  const getCachedDashboard = useCallback((afiliadoId: string) => {
  ...
  const saveCachedDashboard = useCallback((afiliadoId: string, data: DashboardData) => {
  ...
  const getCachedVendas = useCallback((afiliadoId: string) => {
    if (typeof window === "undefined") return null;
  ...
  const saveCachedVendas = useCallback((afiliadoId: string, data: Venda[]) => {
    if (typeof window === "undefined") return;
    const cacheKey = `conta_vendas_cache_${afiliadoId}`;
    const dataKey = `conta_vendas_data_${afiliadoId}`;
    sessionStorage.setItem(cacheKey, Date.now().toString());
  ...
  }, [afiliadoSelecionado, getCachedDashboard, saveCachedDashboard]);

  const applyFilters = useCallback((base: Venda[]) => {
  ...
  }, [allVendas, applyFilters]);

  const fetchVendas = useCallback(async (isBackground: boolean) => {
    ...
    try {
      ...
      // Buscar pagamentos do Asaas usando /api/asaas-data
      const paymentsResponse = await fetch(
        `/api/asaas-data?afiliadoId=${afiliadoSelecionado}&tipo=payments`,
      ...
      // Buscar assinaturas do Asaas usando /api/asaas-data
      const subscriptionsResponse = await fetch(
        `/api/asaas-data?afiliadoId=${afiliadoSelecionado}&tipo=subscriptions`,
        { signal: abortController.signal }
      );
  ...
  }, [afiliadoSelecionado, getCachedVendas, saveCachedVendas]);

  // Quando o afiliado selecionado muda, buscar novos dados
  useEffect(() => {
    if (afiliadoSelecionado) {
      fetchDashboardData(false);
      fetchVendas(false);
    }
  }, [afiliadoSelecionado, fetchDashboardData, fetchVendas]);
  ...
  const afiliadoAtual = useMemo(() => 
    afiliados.find(a => a.id === afiliadoSelecionado),
    [afiliados, afiliadoSelecionado]
  );
  
  const comissaoPercentual = useMemo(() => 
    afiliadoAtual?.porcentagem_comissao || 0.03,
    [afiliadoAtual]
  );

  // Calcular totais baseados nas vendas filtradas
  const totalValorVendas = useMemo(() => 
    vendas.reduce((sum, v) => sum + (v.valor || 0), 0),
    [vendas]
  );
  
  const totalComissao = useMemo(() => 
    totalValorVendas * comissaoPercentual,
    [totalValorVendas, comissaoPercentual]
  );
  ...
  }), [afiliados.length, dashboard]);

  const handleRefresh = useCallback(() => {
    fetchDashboardData(false);
    fetchVendas(false);
    toast.success("Dados atualizados com sucesso!");
  }, [fetchDashboardData, fetchVendas]);
