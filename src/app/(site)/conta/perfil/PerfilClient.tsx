...
interface PerfilClientProps {
  ...
  initialBankData: DadosPix | null
}

export default function PerfilClient({ initialAfiliado, initialBankData }: PerfilClientProps) {
  const router = useRouter()
  const supabase = useMemo(() => createClient(), []);

  const [afiliado, setAfiliado] = useState<Afiliado | null>(initialAfiliado)
  const [loading, setLoading] = useState(!initialAfiliado)
  const [refreshing, setRefreshing] = useState(false)
  const [saving, setSaving] = useState(false)
  const [dadosBancarios, setDadosBancarios] = useState<DadosPix | null>(initialBankData)
  const [uploading, setUploading] = useState(false)

  const [formData, setFormData] = useState({
    nome_completo: initialAfiliado?.nome_completo || '',
    email: initialAfiliado?.email || '',
    cpf_cnpj: initialAfiliado?.cpf_cnpj || '',
    telefone: initialAfiliado?.telefone || '',
    foto_perfil_url: initialAfiliado?.foto_perfil_url || '',
    porcentagem_comissao: initialAfiliado?.porcentagem_comissao || 0.03,
    valor_adesao: initialAfiliado?.valor_adesao || 0,
    meta: initialAfiliado?.meta || 2500,
    ativo: initialAfiliado?.ativo ?? true,
  })

  const [formPix, setFormPix] = useState({
    pix_address_key: initialBankData?.pix_address_key || '',
    operation_type: initialBankData?.operation_type || 'PIX',
    pix_address_key_type: initialBankData?.pix_address_key_type || 'EMAIL',
    description: initialBankData?.description || '',
    accountName: initialBankData?.accountName || '',
    ownerName: initialBankData?.ownerName || '',
    ownerBirthDate: initialBankData?.ownerBirthDate || '',
    cpfCnpj: initialBankData?.cpfCnpj || '',
    agency: initialBankData?.agency || '',
    account: initialBankData?.account || '',
    accountDigit: initialBankData?.accountDigit || '',
    bankAccountType: initialBankData?.bankAccountType || 'CONTA_CORRENTE'
  })
  ...
  const applyAfiliadoData = useCallback((data: Afiliado) => {
    setAfiliado(data)
    setFormData({
  ...
  const applyBankData = useCallback((data: DadosPix | null) => {
  ...
  const getCachedPerfil = useCallback((authId: string) => {
  ...
  const saveCachedPerfil = useCallback((authId: string, data: Afiliado, bank: DadosPix | null) => {
  ...
  const carregarPerfil = useCallback(async (isBackground: boolean) => {
  ...
  }, [applyAfiliadoData, applyBankData, getCachedPerfil, router, saveCachedPerfil, supabase])

  const buscarDadosBancarios = useCallback(async (afiliadoId: string) => {
    try {
  ...
  }, [applyBankData, supabase])

  // Inicializar dados apenas uma vez no mount — evita selects ficarem em "carregando"
  useEffect(() => {
    if (initializedRef.current) return
    initializedRef.current = true
    if (initialAfiliado) {
      applyAfiliadoData(initialAfiliado)
      applyBankData(initialBankData || null)
      setLoading(false)
    } else {
